...
| `PublicController` | `/api/public` | `GET /events/current`、`GET /events`、`GET /events/{eventId}`、`GET /events/{eventId}/tracks`、`GET /events/{eventId}/form-fields`、`GET /projects` |
| `RegistrationController` | `/api` | `POST /registrations` |
| `AdminEventController` | `/api/admin/events` | `GET /current/config`、`GET /{eventId}/config`、`PUT /{eventId}/config` |
...
后台活动配置已进入 MVP 联调；后台报名、用户、项目、邮件仍未实现真实接口。

## 2. 服务端口

| 服务 | 端口 / 路径 | 说明 |
| --- | --- | --- |
| 前端开发服务 | `http://localhost:5173/` | Vue 3 + Vite，开发期通过 proxy 转发 `/api` |
| 后台页面入口 | `http://localhost:5173/root` | Vue Router 后台管理入口 |
...
> **联调说明：** Vite 开发期使用 `frontend/vite.config.js` 代理 `/api -> http://localhost:8080`。生产期可由 Nginx 或 Spring Boot 静态托管处理同源。
...
| `GET` | `/api/admin/events/current/config` | 已实现 | 后台读取当前公开活动聚合配置。 |
| `GET` | `/api/admin/events/{eventId}/config` | 已实现 | 后台读取指定活动聚合配置。 |
| `PUT` | `/api/admin/events/{eventId}/config` | 已实现 | 后台保存活动基础信息、赛道、报名字段；MVP 采用整体替换策略。 |
| `GET` | `/api/public/events/current` | 已实现 | 当前公开活动聚合配置。 |
| `GET` | `/api/public/events/{eventId}` | 已实现 | 前台读取指定活动聚合配置。 |
| `GET` | `/api/public/events/{eventId}/tracks` | 已实现 | 前台读取活动赛道。 |
| `GET` | `/api/public/events/{eventId}/form-fields?targetType=registration` | 已实现 | 前台读取动态报名字段。 |
...
| v0.6 | 2026-07-08 | 新增后台活动聚合配置接口和前台活动详情/赛道/表单字段接口，打通 `5173 -> 8080 -> MySQL` 动态活动配置链路。 |
