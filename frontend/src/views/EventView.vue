...
const event = ref(normalizeApiEvent({ id: route.params.id }))
...
async function loadEvent() {
  ...
  try {
    const data = /^\d+$/.test(routeId) ? await EventApi.detail(routeId) : await EventApi.current()
  ...
  <main>
    <section class="hero event-hero">
      <div class="hero-content">
        ...
        <div class="event-primary-cta">
          ...
          <router-link class="outline-button" :to="`/projects?event=${event.id}`">查看公开作品</router-link>
    ...
    <section class="section" id="requirements" v-if="event.submissionRequirements?.length">
      ...
      <div class="section-cta" v-if="canRegister">
        ...
        <router-link class="outline-button cta-large" :to="`/projects?event=${event.id}`">看看公开作品</router-link>
      </div>
    </section>
  </main>
</template>
