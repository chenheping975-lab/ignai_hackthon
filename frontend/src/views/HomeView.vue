...
onMounted(async () => {
  try {
    ...
    events.value = (res?.list || []).map(normalizeEvent)
  } catch {
    events.value = []
  }
})
...
function buildStatusText(data) {
  ...
  if (data.status === 'running') return '进行中'
  if (data.status === 'archived') return '已归档'
  return data.registrationOpen ? '报名开放' : '已发布'
}
...
const featured = computed(() => {
  const open = events.value.find(e => e.registrationOpen || e.status === 'running')
  ...
  <main class="home">
    ...
    <section class="home-hero">
      ...
      <div class="hero-actions">
        ...
        <router-link class="outline-button" to="/projects">浏览公开作品</router-link>
