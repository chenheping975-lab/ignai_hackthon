...
这是一个无依赖静态原型，主视觉沿用 IGNAI 官网的暖纸底、黑色 CTA、橙红火炬和品牌 IP 色。前台链路拆成三层：首页传达活动精神和活动入口，单场活动页承接报名，单场作品平台承接展示、筛选和点赞。
...
按钮 / 交互自动化测试基于 [Playwright Test](https://playwright.dev)，覆盖公共首页主题切换、hero CTA、活动卡片进入详情、单场活动报名、作品平台筛选点赞、Reveal 入场、跑马灯，以及 `/root/` 后台管理 MVP 的 root 初始化、报名审核、问卷字段编辑、项目公开和赛事预览联动。
...
- 首页活动精神、活动卡片和 IGNAI 官网友链。
- 单场活动页：活动状态、赛道、报名表单、提交要求和作品平台入口。
- 单场作品平台：赛道筛选、票数 / 最新 / 精选排序、点赞、附件标签。
- 官网和对标网站友链。
- 登录 / 注册后的账号状态、退出、切换账号。
- 首页不再强行展示“行动流程”栏目，流程跟随单场活动页。
- 端口和 API 摘要。
- `/root/` 后台管理 MVP：概览、活动管理、可编辑报名问卷、报名管理、项目看板、赛事预览、邮件中心。
...
修复阻尼感、重构前台信息架构、完善后台字段编辑器。

- `[data-reveal]` 动画从 0.8s expo 缩短至 0.28s ease-out，去掉 blur 和 filter，解决滚动粘滞感。
- 首页删掉 `#works-entry`（流程讲解栏目）和导航「作品平台」；活动卡片只保留「进入活动」一个 CTA。
- `event.html` hero 的「查看作品平台」改为 `text-link`，作品平台入口下沉到 `status-next` 区域；按活动阶段显示不同引导（报名→去报名；showcase→进入作品平台）。
- 新增 Content Maker Athon（showcase 阶段）mock 活动，扩展至 3 场活动 + 9 个项目。
- 后台字段编辑器重做为两行布局（标题+必填+删除 / 题型+占位+选项），切换题型只局部重渲染当前卡片的选项区块，不再全量 re-render 导致输入框失焦。
- 新增问题后自动聚焦标题输入框；扩充后台 mock 数据至 6 条报名、5 个项目。
- 参考 [aihacktour.com](https://aihacktour.com/) 链路：首页只讲精神 + 活动列表，不放作品/项目入口。

涉及文件：`index.html`、`event.html`、`assets/css/styles.css`、`assets/js/event-chain.js`、`root/root.js`、`root/root.css`、`tests/buttons.spec.ts`、`docs/TODO-UI信息架构.md`。

### v0.7.0 — 2026-07-08

重构前台链路并完善后台问卷编辑器。

- 首页从”为什么 / 行动 / 流程 / 作品”的单页流程说明，调整为”活动精神 + 可报名黑客松活动卡片”。
- 新增 `event.html` 单场活动页，展示活动状态、赛道、报名表单、提交要求和作品平台入口。
- 新增 `projects.html` 单场作品平台，支持按赛道筛选、按精选 / 票数 / 最新排序和 mock 点赞。
- 新增 `assets/js/event-chain.js`，集中维护 mock 活动、报名字段和项目数据。
- `/root/` 活动管理里的报名问题从静态列表升级为字段编辑器，支持标题、类型、必填、占位提示和选项编辑。
- 更新 Playwright 测试，覆盖前台三层链路和后台问卷编辑。
...
- 新增后台 Shell：左侧导航、顶部活动切换、root 状态和飞书同步状态。
- 新增概览 Dashboard：指标、今日待处理、赛事窗口。
- 新增活动管理：报名问题列表、动态新增问题、参赛者表单预览。
- 新增报名管理：筛选、报名表、详情抽屉、AI 初筛、审核通过并生成邮件草稿。
- 新增项目看板：项目状态、附件标签、公开 / 精选切换。
- 新增整体赛事预览：按身份和阶段预览访客、参赛者、队长看到的行动入口。
- 新增 Playwright 后台测试，当前测试总数 15。

涉及文件：`root/index.html`、`root/root.css`、`root/root.js`、`tests/root-admin.spec.ts`、`README.md`。
